...
season_narm<-winter %>%
  filter(is.finite(winter[,var.num]),is.finite(soiltemp_mean))
...
write("
      ...
      as[i] ~ normal(gamma0[tea_type_site[i]] + gamma1[tea_type_site[i]]*temp_pred_site[i], sigma_overall); 
      ...
      else aMeanRegion[i] ~ normal(gamma0[tea_type_region[i]] + gamma1[tea_type_region[i]]*temp_pred_region[i], sigma_overall); 
      }
      ...
      for (i in 1:Nxhat){
      for (j in 1:NTea){
      preds[i,j] = (gamma0[j] + gamma1[j]*xhat1[i]); //predictions 
      }
      }
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/soiltemp_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 18), algorithm = "NUTS") 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/soiltemp_preds_winter.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/soiltemp_fits_winter.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/soiltemp_med_winter.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=soiltemp+soiltemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=soiltemp_mean+soiltemp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=soiltemp+soiltemp_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
(soiltemp_only_winter<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = soiltemp_mean+soiltemp_cent_amount, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = soiltemp_mean+soiltemp_cent_amount, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=soiltemp+soiltemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=soiltemp_mean+soiltemp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=soiltemp+soiltemp_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(soiltemp_only_winter, file = "scripts/users/hthomas/Tea/soiltemp_only_winter.Rdata")

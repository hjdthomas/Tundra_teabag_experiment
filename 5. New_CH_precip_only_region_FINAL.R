...
write("
      ...
      real<lower=-2,upper=2> gamma2[NTea];  // slope of CHELSA_summer_precip - loss relationship
      real<lower=-2,upper=2> gamma3[NTea];  // temperature - CHELSA_summer_precip interaction
      real<lower=-2,upper=2> gamma4[NTea];  // temperature - CHELSA_summer_precip interaction
      ...
      //Relationship between mass loss at the region level and temperature and CHELSA_summer_precip, per tea type
      ...
      ","scripts/users/hthomas/Tea/CHELSA_summer_precip_loss_3.stan")



stanc('scripts/users/hthomas/Tea/CHELSA_summer_precip_loss_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/CHELSA_summer_precip_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
predsout.space$CHELSA_summer_precip <- rep(jags.dat$xhat1, each=2)
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_precip_preds_summer.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_precip_fits_summer.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/CHELSA_summer_precip_med_summer.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
lm <- lmer(Loss ~ CHELSA_summer_precip * Tea_Type + (CHELSA_summer_precip|ESA_cell/Site/Plot), data = season_narm_r)
out<-as.data.frame(effect(c("CHELSA_summer_precip","Tea_Type"),lm))



(CHELSA_summer_precip_only_summer<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = CHELSA_summer_precip+CHELSA_summer_precip_cent_amount, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = CHELSA_summer_precip+CHELSA_summer_precip_cent_amount, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(CHELSA_summer_precip_only_summer, file = "scripts/users/hthomas/Tea/CHELSA_summer_precip_only_summer.Rdata")

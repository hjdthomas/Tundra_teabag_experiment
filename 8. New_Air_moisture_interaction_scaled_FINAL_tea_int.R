...
airtemp_cent_amount <- attr(scale(season_narm_r$airtemp_mean, center = TRUE, scale = TRUE), 'scaled:center')
airtemp_scale_amount <- attr(scale(season_narm_r$airtemp_mean, center = TRUE, scale = TRUE), 'scaled:scale')
season_narm_r$airtemp_mean<-scale(season_narm_r$airtemp_mean, center = TRUE, scale = TRUE)
...
xhats <- expand.grid(xhat1=seq(min_airtemp, max_airtemp,by=0.01), xhat2=scale(c(LQ_moist,UQ_moist,mean_moist,med_moist), center = TRUE, scale = TRUE),xhat3 = mean_burial) #AB: predicting soil airtemp at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
predsout.space$TempBT <- predsout.space$airtemp  * airtemp_scale_amount+ airtemp_cent_amount 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_moisture_preds_tea_int_scaled.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_moisture_fits_tea_int_scaled.Rdata")

#Anne graph
pdf("scripts/users/hthomas/Output_Images/Tea/airtemp_moisture_scaled.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp * airtemp_scale_amount+airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(Moisture)),alpha=0.2)+
  geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=airtemp_mean* airtemp_scale_amount+airtemp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
  geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=airtemp_mean* airtemp_scale_amount+airtemp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp* airtemp_scale_amount+airtemp_cent_amount,y=mean, colour=factor(Tea_Type):factor(Moisture)), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
(airtemp_moist_tea_int_scaled<-ggplot()+
    geom_ribbon(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp * airtemp_scale_amount+airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(Moisture)),alpha=0.2)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=airtemp_mean* airtemp_scale_amount+airtemp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=airtemp_mean* airtemp_scale_amount+airtemp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp* airtemp_scale_amount+airtemp_cent_amount,y=mean, colour=factor(Tea_Type):factor(Moisture)), alpha=0.8, lwd = 1.5)+
...
save(airtemp_moist_tea_int, file = "scripts/users/hthomas/Tea/airtemp_moist_tea_int_scaled.Rdata")

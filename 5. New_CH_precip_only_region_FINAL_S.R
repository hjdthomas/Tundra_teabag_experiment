...
tea<-subset(tea,Tea_Type == "Green")

tea <- tea %>%
  group_by(ESA_cell,Site,Plot) %>%
  distinct(TBI_S, .keep_all = TRUE)
...
season_narm<-summer %>%
  filter(is.finite(TBI_S),is.finite(CHELSA_summer_precip))
...
season_narm_r <- ddply(season_narm_r, c("ESA_cell","Site","Plot","Tea_Type"), transform, NObsPlot = length(TBI_S))
...
# AB: REMOVE MISSING VALUES OF SOIL CHELSA_summer_precip AND TEMPERATURE FOR THE CHELSA_summer_precip X TEMPERATURE INTERACTION MODEL
...
min_soil<-min(season_narm_r$CHELSA_summer_precip,na.rm=TRUE)
max_soil<-max(season_narm_r$CHELSA_summer_precip,na.rm=TRUE)
...
xhats <- expand.grid(xhat1=seq(min_CHELSA_summer_precip, max_CHELSA_summer_precip,by=0.01), xhat3 = mean_burial) #AB: predicting soil CHELSA_summer_precip at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
write("
      ...
      vector[Nobs] traitobs; //Mass TBI_S
      ...
      real<lower=-2,upper=2> gamma0;  // intercept of relationship between mass TBI_S and temp change 
      real<lower=-2,upper=2> gamma1;  // slope of temperature - TBI_S relationship
      real<lower=-2,upper=2> gamma2;  // slope of CHELSA_summer_precip - TBI_S relationship
      real<lower=-2,upper=2> gamma3;  // temperature - CHELSA_summer_precip interaction
      real<lower=-2,upper=2> gamma4;  // temperature - CHELSA_summer_precip interaction
      
      real<lower=0,upper=5> sigma_overall; //Error around TBI_S- temp relationship
      ...
      //Relationship between mass TBI_S at the region level and temperature and CHELSA_summer_precip, per tea type
      ...
      ","scripts/users/hthomas/Tea/CHELSA_summer_precip_TBI_S_3.stan")



stanc('scripts/users/hthomas/Tea/CHELSA_summer_precip_TBI_S_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/CHELSA_summer_precip_TBI_S_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
#gamma1 = temperature, gamma2 = CHELSA_summer_precip, gamma3 = temp X CHELSA_summer_precip interaction (for each tea type)
...
# ggplot(plot.compare)+
#   geom_point(aes(x=rawTBI_S,y=StanEst,colour=Tea_Type))
...
predsout.space$CHELSA_summer_precip <-jags.dat$xhat1
predsout.space$MostureBT <- predsout.space$CHELSA_summer_precip + CHELSA_summer_precip_cent_amount
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_precip_preds_summer_TBI_S.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_precip_fits_summer_TBI_S.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/CHELSA_summer_precip_med_summer_TBI_S.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
  theme_classic()+
  #coord_cartesian(y = c(0,1))+
  #scale_colour_manual(values = c("darkgreen","darkgreen"), name = "Tea Type")+
  #scale_fill_manual(values = c("darkgreen","darkgreen"), name = "Tea Type")+
  #scale_linetype_manual(values = c("dashed","solid"), name = "CHELSA_summer_precip", labels = c("low","high"))+
  labs(x = "Soil Temperature (°C)", y = "Mass TBI_S (%)")+
  theme(legend.position = "none")
dev.off()
library(effects)
lm <- lmer(TBI_S ~ CHELSA_summer_precip  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("CHELSA_summer_precip"),lm))

(CHELSA_summer_precip_only_summer_TBI_S<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = CHELSA_summer_precip+CHELSA_summer_precip_cent_amount, ymin = lower, ymax = upper),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = CHELSA_summer_precip+CHELSA_summer_precip_cent_amount, y = fit),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=CHELSA_summer_precip+CHELSA_summer_precip_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(CHELSA_summer_precip_only_summer_TBI_S, file = "scripts/users/hthomas/Tea/CHELSA_summer_precip_only_summer_TBI_S.Rdata")

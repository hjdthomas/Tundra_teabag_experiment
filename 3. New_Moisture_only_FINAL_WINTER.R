...
season_narm<-winter %>%
  filter(is.finite(winter[,var.num]),is.finite(moisture_mean))
...
write("
      ...
      as[i] ~ normal(gamma0[tea_type_site[i]] + gamma1*temp_pred_site[i] + gamma4*days_pred_site[i], sigma_overall); 
      ...
      else aMeanRegion[i] ~ normal(gamma0[tea_type_region[i]] + gamma1*temp_pred_region[i] + gamma4*days_pred_region[i] , sigma_overall); 
      }
      ...
      for (i in 1:Nxhat){
      for (j in 1:NTea){
      preds[i,j] = (gamma0[j] + gamma1*xhat1[i] + gamma4*xhat3[i]); //predictions 
      }
      }
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/moisture_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 18), algorithm = "NUTS") 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/moisture_preds_winter.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/moisture_fits_winter.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/moisture_med_winter.pdf", width = 3, height = 3)
ggplot()+
...
save(moisture_only_winter, file = "scripts/users/hthomas/Tea/moisture_only_winter.Rdata")

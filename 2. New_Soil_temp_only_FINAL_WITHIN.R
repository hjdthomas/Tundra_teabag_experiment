...
season_narm_r <- subset(season_narm_r, MultiplePlots == 1 | MultipleSites == 1 | envlevel > 0)
...
season_narm_r<-subset(season_narm_r,MultiplePlots>0 | MultipleSites > 0)
...
season_narm_r_plots<-season_narm_r %>%
  group_by(PlotNum) %>%
  summarise(soiltemp_mean_plot = mean(soiltemp_mean),
            soiltemp_sd_plot = sd(soiltemp_mean))

season_narm_r$soiltemp_mean_plot<-season_narm_r_plots$soiltemp_mean_plot[match(season_narm_r$PlotNum, season_narm_r_plots$PlotNum)]
season_narm_r$soiltemp_sd_plot<-season_narm_r_plots$soiltemp_sd_plot[match(season_narm_r$PlotNum, season_narm_r_plots$PlotNum)]

season_narm_r$soiltemp_sd_plot[season_narm_r$soiltemp_sd_plot==0] <- mean(season_narm_r$soiltemp_sd_plot[season_narm_r$soiltemp_sd_plot>0],na.rm = T)
season_narm_r$soiltemp_sd_plot[is.na(season_narm_r$soiltemp_sd_plot)] <- 0.01
...
season_narm_r<-season_narm_r %>%
  group_by(PlotNum) %>%
  mutate(plotDays = mean(Days),
         plotDays_sd = sd(Days))

season_narm_r$plotDays_sd[season_narm_r$plotDays_sd==0 | is.na(season_narm_r$plotDays_sd)] <- 0.001
...
xhats <- expand.grid(xhat1=seq(min_soiltemp, max_soiltemp,by=0.01), xhat3 = mean_burial) #AB: predicting air soiltemp at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
jags.dat<-list(
  ...
  tea_type_plot=ifelse(season_narm_r$Tea_Type[!duplicated(season_narm_r$PlotNum)]=="Green", 1, 2),
...
write("
      ...
      int<lower=0> NPlotDays; //No. of days
      ...
      int<lower=1,upper=2> tea_type_plot[NPlot]; //Tea type (1=Green, 2=Rooibos)
      ...
      int<lower=0,upper=1> multobs_lsite[NSite]; //Are sites nested in region (all obs)
      ...
      int<lower=0,upper=1> multplots_lsite[NSite]; //Are plots nested in site  (no plots)
      int<lower=0,upper=2> obs_envlevel[Nobs];
      int<lower=0,upper=2> plot_envlevel[NPlot];
      ...
      vector[NPlot] temp_mean_plot; //Temperature (unique regions)
      vector[NPlot] temp_sd_plot; //Temperature SD (unique regions)
      ...
      vector[NPlot] plotDays; //
      vector[NPlot] plotDays_sd; //
      ...
      parameters {
      real<lower=-3,upper=3> ap[NPlot];  // Region effect
      real<lower=-5,upper=5> as[NSite];
      real<lower=-5,upper=5> ar[NRegion];
      ...
      vector[NPlot] temp_pred_plot;
      vector[NPlot] days_pred_plot;
      ...
      if(multsites_lobs[i] == 0 || obs_envlevel[i] >1)
      ass[i] = 0;
      else ass[i] = as[Site[i]];

      
      if(multsites_lobs[i]==1 )
      arr[i] = 0;
      else arr[i] = ar[Region[i]];
      ...
      for (i in 1:NPlot){
      temp_pred_plot[i] ~ normal(temp_mean_plot[i], temp_sd_plot[i]); //temp_mean_plot and temp_sd are given as data
      days_pred_plot[i] ~ normal(plotDays[i], plotDays_sd[i]); //temp_mean_plot and temp_sd are given as data
      }

      for (i in 1:NSite){
      temp_pred_site[i] ~ normal(temp_mean_site[i], temp_sd_site[i]); //temp_mean_plot and temp_sd are given as data
      days_pred_site[i] ~ normal(SiteDays[i], SiteDays_sd[i]); //temp_mean_plot and temp_sd are given as data
      }

      for (i in 1:NRegion){
      temp_pred_region[i] ~ normal(temp_mean_region[i], temp_sd_region[i]); //temp_mean_plot and temp_sd are given as data
      days_pred_region[i] ~ normal(RegionDays[i], RegionDays_sd[i]); //temp_mean_plot and temp_sd are given as data
      }
      ...
      for (i in 1:NPlot){
      if(multplots_lplot[i] == 1 && plot_envlevel[i] >1)
      ap[i] ~ normal(gamma0[tea_type_plot[i]] + gamma1[tea_type_plot[i]]*temp_pred_plot[i] + gamma4[tea_type_plot[i]]*days_pred_plot[i], sigma_overall); 
      else ap [i] ~ normal(0, sigma_plot);
      }
...
initsA <- list(ar=rep(0.6,jags.dat$NRegion),ap=rep(0.6,jags.dat$NPlot), as=rep(0.6,jags.dat$NSite))
initsB <- list(ar=rep(0.3,jags.dat$NRegion),ap=rep(0.3,jags.dat$NPlot), as=rep(0.3,jags.dat$NSite))
...
predsout.space$MostureBT <- predsout.space$soiltemp + soiltemp_cent_amount
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/soiltemp_preds_within.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/soiltemp_fits_within.Rdata")


pdf("scripts/users/hthomas/Output_Images/Tea/soiltemp_within.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=soiltemp,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=soiltemp_mean,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=soiltemp,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
lm <- lmer(Loss ~ soiltemp_mean * Tea_Type + (soiltemp_mean|ESA_cell/Site/Plot), data = season_narm_r)
out<-as.data.frame(effect(c("soiltemp_mean","Tea_Type"),lm))


(soiltemp_within<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = soiltemp_mean, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = soiltemp_mean, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=soiltemp,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=soiltemp_mean,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=soiltemp,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(soiltemp_within, file = "scripts/users/hthomas/Tea/soiltemp_within.Rdata")

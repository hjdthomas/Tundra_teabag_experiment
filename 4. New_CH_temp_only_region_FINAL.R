...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_temp_preds_summer.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_temp_fits_summer.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/CHELSA_summer_temp_med_summer.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
lm <- lmer(Loss ~ CHELSA_summer_temp * Tea_Type + (CHELSA_summer_temp|ESA_cell/Site/Plot), data = season_narm_r)
out<-as.data.frame(effect(c("CHELSA_summer_temp","Tea_Type"),lm))



(CHELSA_summer_temp_only_summer<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = CHELSA_summer_temp+CHELSA_summer_temp_cent_amount, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = CHELSA_summer_temp+CHELSA_summer_temp_cent_amount, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(CHELSA_summer_temp_only_summer, file = "scripts/users/hthomas/Tea/CHELSA_summer_temp_only_summer.Rdata")

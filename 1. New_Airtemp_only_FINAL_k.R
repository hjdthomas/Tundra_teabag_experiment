...
season_narm<-summer %>%
  filter(is.finite(TBI_k),is.finite(airtemp_mean))
...
write("
      ...
      real<lower=-2,upper=2> gamma2;  // slope of airtemp - TBI_k relationship
      ...
      //Relationship between mass TBI_k at the region level and temperature and airtemp, per tea type
      ...
      ","scripts/users/hthomas/Tea/airtemp_TBI_k_3.stan")



stanc('scripts/users/hthomas/Tea/airtemp_TBI_k_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/airtemp_TBI_k_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_preds_summer_TBI_k.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_fits_summer_TBI_k.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/airtemp_med_summer_TBI_k.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "#006400",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=airtemp_mean+airtemp_cent_amount,y=TBI_k),colour="#006400",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,y=mean), colour = "#006400", alpha=0.8, lwd = 1.5)+
  theme_classic()+
  #coord_cartesian(y = c(0,1))+
  #scale_colour_manual(values = c("#006400","#006400"), name = "Tea Type")+
  #scale_fill_manual(values = c("#006400","#006400"), name = "Tea Type")+
  #scale_linetype_manual(values = c("dashed","solid"), name = "airtemp", labels = c("low","high"))+
  labs(x = "Soil Temperature (°C)", y = "Mass TBI_k (%)")+
  theme(legend.position = "none")
dev.off()
library(effects)
lm <- lmer(TBI_k ~ airtemp_mean  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("airtemp_mean"),lm))

(airtemp_only_summer_TBI_k<-ggplot()+
...
save(airtemp_only_summer_TBI_k, file = "scripts/users/hthomas/Tea/airtemp_only_summer_TBI_k.Rdata")

...
season_narm_r$envlevel<-ifelse(season_narm_r$CHELSA_year_precip_var_level=="Region",0,
                               ifelse(season_narm_r$CHELSA_year_precip_var_level=="Site",1,2))

#Add env.levels (alternative)
#Add env.levels (new - based on nestedness)
env.levels<- season_narm_r %>%
  select(CHELSA_year_precip,ESA_cell,Site,Plot) 

season_narm_r$envlevel<-0

env.levels2<-ddply(env.levels, c("ESA_cell"), summarise, n.plots = length(unique(CHELSA_year_precip)))
...
env.levels2<-ddply(env.levels, c("ESA_cell","Site"), summarise, n.plots = length(unique(CHELSA_year_precip)))
...
# AB: REMOVE MISSING VALUES OF SOIL CHELSA_year_precip AND TEMPERATURE FOR THE CHELSA_year_precip X TEMPERATURE INTERACTION MODEL
season_narm_r <- season_narm_r[!is.na(season_narm_r$CHELSA_year_precip),]
...
CHELSA_year_precip_cent_amount <- attr(scale(season_narm_r$CHELSA_year_precip, center = TRUE, scale = FALSE), 'scaled:center')
season_narm_r$CHELSA_year_precip<-scale(season_narm_r$CHELSA_year_precip, center = TRUE, scale = FALSE)
...
season_narm_r_sites<-season_narm_r %>%
  group_by(SiteNum) %>%
  summarise(CHELSA_year_precip_site = mean(CHELSA_year_precip),
            CHELSA_year_precip_sd_site = sd(CHELSA_year_precip))

season_narm_r$CHELSA_year_precip_site<-season_narm_r_sites$CHELSA_year_precip_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]
season_narm_r$CHELSA_year_precip_sd_site<-season_narm_r_sites$CHELSA_year_precip_sd_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]

season_narm_r$CHELSA_year_precip_sd_site[season_narm_r$CHELSA_year_precip_sd_site==0 ] <- mean(season_narm_r$CHELSA_year_precip_sd_site[season_narm_r$CHELSA_year_precip_sd_site>0],na.rm = T)
season_narm_r$CHELSA_year_precip_sd_site[is.na(season_narm_r$CHELSA_year_precip_sd_site)] <- 0.01
...
season_narm_r_regions<-season_narm_r %>%
  group_by(RegionNum) %>%
  summarise(CHELSA_year_precip_region = mean(CHELSA_year_precip),
            CHELSA_year_precip_sd_region = sd(CHELSA_year_precip))

season_narm_r$CHELSA_year_precip_region<-season_narm_r_regions$CHELSA_year_precip_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
season_narm_r$CHELSA_year_precip_sd_region<-season_narm_r_regions$CHELSA_year_precip_sd_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]

season_narm_r$CHELSA_year_precip_sd_region[season_narm_r$CHELSA_year_precip_sd_region==0] <- mean(season_narm_r$CHELSA_year_precip_sd_region[season_narm_r$CHELSA_year_precip_sd_region>0],na.rm = T)
season_narm_r$CHELSA_year_precip_sd_region[is.na(season_narm_r$CHELSA_year_precip_sd_region)] <- 0.01
...
min_soil<-min(season_narm_r$CHELSA_year_precip,na.rm=TRUE)
max_soil<-max(season_narm_r$CHELSA_year_precip,na.rm=TRUE)
min_CHELSA_year_precip<-min(season_narm_r$CHELSA_year_precip,na.rm=TRUE)
max_CHELSA_year_precip<-max(season_narm_r$CHELSA_year_precip,na.rm=TRUE)

xhats <- expand.grid(xhat1=seq(min_CHELSA_year_precip, max_CHELSA_year_precip,by=0.01), xhat3 = mean_burial) #AB: predicting soil CHELSA_year_precip at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
write("
      ...
      real<lower=-2,upper=2> gamma2[NTea];  // slope of CHELSA_year_precip - loss relationship
      real<lower=-2,upper=2> gamma3[NTea];  // temperature - CHELSA_year_precip interaction
      real<lower=-2,upper=2> gamma4[NTea];  // temperature - CHELSA_year_precip interaction
      ...
      //Relationship between mass loss at the region level and temperature and CHELSA_year_precip, per tea type
      ...
      ","scripts/users/hthomas/Tea/CHELSA_year_precip_loss_3.stan")



stanc('scripts/users/hthomas/Tea/CHELSA_year_precip_loss_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/CHELSA_year_precip_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
#gamma1 = temperature, gamma2 = CHELSA_year_precip, gamma3 = temp X CHELSA_year_precip interaction (for each tea type)
...
predsout.space$CHELSA_year_precip <- rep(jags.dat$xhat1, each=2)
predsout.space$MostureBT <- predsout.space$CHELSA_year_precip + CHELSA_year_precip_cent_amount
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_year_precip_preds_year.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_year_precip_fits_year.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/CHELSA_year_precip_med_year.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=CHELSA_year_precip+CHELSA_year_precip_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=CHELSA_year_precip+CHELSA_year_precip_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=CHELSA_year_precip+CHELSA_year_precip_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
  coord_cartesian(y = c(0,1))+
  scale_colour_manual(values = c("darkgreen","red3"), name = "Tea Type")+
  scale_fill_manual(values = c("darkgreen","red3"), name = "Tea Type")+
  scale_linetype_manual(values = c("dashed","solid"), name = "CHELSA_year_precip", labels = c("low","high"))+
...
lm <- lmer(Loss ~ CHELSA_year_precip * Tea_Type + (CHELSA_year_precip|ESA_cell/Site/Plot), data = season_narm_r)
out<-as.data.frame(effect(c("CHELSA_year_precip","Tea_Type"),lm))



(CHELSA_year_precip_only_year<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = CHELSA_year_precip+CHELSA_year_precip_cent_amount, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = CHELSA_year_precip+CHELSA_year_precip_cent_amount, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=CHELSA_year_precip+CHELSA_year_precip_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=CHELSA_year_precip+CHELSA_year_precip_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=CHELSA_year_precip+CHELSA_year_precip_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(CHELSA_year_precip_only_year, file = "scripts/users/hthomas/Tea/CHELSA_year_precip_only_year.Rdata")

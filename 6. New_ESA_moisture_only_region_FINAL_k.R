...
season_narm<-summer %>%
  filter(is.finite(TBI_k),is.finite(ESA_moisture))
...
write("
      ...
      real<lower=-2,upper=2> gamma2;  // slope of ESA_moisture - TBI_k relationship
      ...
      //Relationship between mass TBI_k at the region level and temperature and ESA_moisture, per tea type
      ...
      ","scripts/users/hthomas/Tea/ESA_moisture_TBI_k_3.stan")



stanc('scripts/users/hthomas/Tea/ESA_moisture_TBI_k_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/ESA_moisture_TBI_k_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_preds_summer_TBI_k.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_fits_summer_TBI_k.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/ESA_moisture_med_summer_TBI_k.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "red3",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=TBI_k),colour="red3",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=mean), colour = "red3", alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
dev.off()
library(effects)
lm <- lmer(TBI_k ~ ESA_moisture  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("ESA_moisture"),lm))

(ESA_moisture_only_summer_TBI_k<-ggplot()+
...
save(ESA_moisture_only_summer_TBI_k, file = "scripts/users/hthomas/Tea/ESA_moisture_only_summer_TBI_k.Rdata")

...
season_narm<-summer %>%
  filter(is.finite(TBI_S),is.finite(airtemp_mean))
...
write("
      ...
      real<lower=-2,upper=2> gamma2;  // slope of airtemp - TBI_S relationship
      real<lower=-2,upper=2> gamma3;  // temperature - airtemp interaction
      real<lower=-2,upper=2> gamma4;  // temperature - airtemp interaction
      ...
      //Relationship between mass TBI_S at the region level and temperature and airtemp, per tea type
      ...
      ","scripts/users/hthomas/Tea/airtemp_TBI_S_3.stan")



stanc('scripts/users/hthomas/Tea/airtemp_TBI_S_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/airtemp_TBI_S_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
predsout.space$airtemp <-jags.dat$xhat1
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_preds_summer_TBI_S.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_fits_summer_TBI_S.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/airtemp_med_summer_TBI_S.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "#8B2323",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=airtemp_mean+airtemp_cent_amount,y=TBI_S),colour="#8B2323",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,y=mean), colour = "#8B2323", alpha=0.8, lwd = 1.5)+
  theme_classic()+
  #coord_cartesian(y = c(0,1))+
  #scale_colour_manual(values = c("#8B2323","#8B2323"), name = "Tea Type")+
  #scale_fill_manual(values = c("#8B2323","#8B2323"), name = "Tea Type")+
  #scale_linetype_manual(values = c("dashed","solid"), name = "airtemp", labels = c("low","high"))+
  labs(x = "Soil Temperature (°C)", y = "Mass TBI_S (%)")+
  theme(legend.position = "none")
dev.off()
library(effects)
lm <- lmer(TBI_S ~ airtemp_mean  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("airtemp_mean"),lm))

(airtemp_only_summer_TBI_S<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = airtemp_mean+airtemp_cent_amount, ymin = lower, ymax = upper),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = airtemp_mean+airtemp_cent_amount, y = fit),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "#8B2323",alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=airtemp_mean+airtemp_cent_amount,y=TBI_S),colour="#8B2323",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=airtemp+airtemp_cent_amount,y=mean), colour = "#8B2323", alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(airtemp_only_summer_TBI_S, file = "scripts/users/hthomas/Tea/airtemp_only_summer_TBI_S.Rdata")

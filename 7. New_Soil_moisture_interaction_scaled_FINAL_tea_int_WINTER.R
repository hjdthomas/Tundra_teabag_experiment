...
write("
      ...
      parameters {
      real<lower=-3,upper=3> aMeanSite[NSite];  // Region effect
      ...
      for (i in 1:NSite){
      temp_pred_site[i] ~ normal(temp_mean_site[i], temp_sd_site[i]); //temp_mean_site and temp_sd are given as data
      moist_pred_site[i] ~ normal(moist_mean_site[i], moist_sd_site[i]); //temp_mean_site and temp_sd are given as data
      days_pred_site[i] ~ normal(SiteDays[i], SiteDays_sd[i]); //temp_mean_site and temp_sd are given as data
      }
      ...
      for (i in 1:NSite){
      aMeanSite[i] ~ normal(gamma0[tea_type_site[i]] + gamma1*temp_pred_site[i] + gamma2*moist_pred_site[i], sigma_overall); 
      }
      ...
      for (i in 1:Nxhat){
      for (j in 1:NTea){
      preds[i,j] = (gamma0[j] + gamma1*xhat1[i] + gamma2*xhat2[i]); //predictions 
      }
      }
...
initsA <- list(ap=rep(0.6,jags.dat$NPlot), aMeanRegion=rep(0.6,jags.dat$NRegion),aMeanSite=rep(0.6,jags.dat$NSite))
initsB <- list(ap=rep(0.3,jags.dat$NPlot), aMeanRegion=rep(0.3,jags.dat$NRegion),aMeanSite=rep(0.3,jags.dat$NSite))
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/temp_moisture_preds_tea_int_winter.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/temp_moisture_fits_tea_int_winter.Rdata")

#Anne graph
pdf("scripts/users/hthomas/Output_Images/Tea/soiltemp_moisture_winter.pdf", width = 3, height = 3)
ggplot()+
...
library(effects)
lm <- lmer(Loss ~ Tea_Type + soiltemp_mean + moisture_mean  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("soiltemp_mean"),lm))


(temp_moist_tea_int_winter<-ggplot()+
    geom_ribbon(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=soiltemp * soiltemp_scale_amount+soiltemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(Moisture)),alpha=0.2)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=soiltemp_mean* soiltemp_scale_amount+soiltemp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=soiltemp_mean* soiltemp_scale_amount+soiltemp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=soiltemp* soiltemp_scale_amount+soiltemp_cent_amount,y=mean, colour=factor(Tea_Type):factor(Moisture)), alpha=0.8, lwd = 1.5)+
...
save(temp_moist_tea_int_winter, file = "scripts/users/hthomas/Tea/temp_moist_tea_int_winter.Rdata")

...
write("
      ...
      as[i] ~ normal(gamma0[tea_type_site[i]] + gamma1[tea_type_site[i]]*temp_pred_site[i] + gamma2[tea_type_site[i]]*moist_pred_site[i] + gamma3*temp_pred_site[i]*moist_pred_site[i] + gamma4[tea_type_site[i]]*days_pred_site[i], sigma_overall); 
      ...
      else aMeanRegion[i] ~ normal(gamma0[tea_type_region[i]] + gamma1[tea_type_region[i]]*temp_pred_region[i] + gamma2[tea_type_region[i]]*moist_pred_region[i] + gamma3*temp_pred_region[i]*moist_pred_region[i] + gamma4[tea_type_region[i]]*days_pred_region[i], sigma_overall); 
      }
      ...
      for (i in 1:Nxhat){
      for (j in 1:NTea){
      preds[i,j] = (gamma0[j] + gamma1[j]*xhat1[i] + gamma2[j]*xhat2[i] + gamma3*xhat1[i]*xhat2[i] + gamma4[j]*xhat3[i]); //predictions 
      }
      }
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/soiltemp_loss_3.stan', data = jags.dat, init=inits, iter = 100000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/soiltemp_moisture_preds_int_scaled_alt.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/soiltemp_moisture_fits_int_scaled_alt.Rdata")

#Anne graph
pdf("scripts/users/hthomas/Output_Images/Tea/soiltemp_moisture_scaled.pdf", width = 3, height = 3)
ggplot()+
...
(soiltemp_moist_tea_int_scaled<-ggplot()+
    geom_ribbon(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=soiltemp * soiltemp_scale_amount+soiltemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(Moisture)),alpha=0.2)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=soiltemp_mean* soiltemp_scale_amount+soiltemp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=soiltemp_mean* soiltemp_scale_amount+soiltemp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=soiltemp* soiltemp_scale_amount+soiltemp_cent_amount,y=mean, colour=factor(Tea_Type):factor(Moisture)), alpha=0.8, lwd = 1.5)+
...
save(soiltemp_moist_tea_int_scaled, file = "scripts/users/hthomas/Tea/soiltemp_moist_tea_int_scaled.Rdata")

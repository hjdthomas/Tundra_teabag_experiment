...
season_narm<-winter %>%
  filter(is.finite(winter[,var.num]),is.finite(ESA_moisture_winter))
...
season_narm_r$envlevel<-ifelse(season_narm_r$ESA_moisture_winter_var_level=="Region",0,
                               ifelse(season_narm_r$ESA_moisture_winter_var_level=="Site",1,2))

#Add env.levels (alternative)
#Add env.levels (new - based on nestedness)
env.levels<- season_narm_r %>%
  select(ESA_moisture_winter,ESA_cell,Site,Plot) 

season_narm_r$envlevel<-0

env.levels2<-ddply(env.levels, c("ESA_cell"), summarise, n.plots = length(unique(ESA_moisture_winter)))
...
env.levels2<-ddply(env.levels, c("ESA_cell","Site"), summarise, n.plots = length(unique(ESA_moisture_winter)))
...
# AB: REMOVE MISSING VALUES OF SOIL ESA_moisture_winter AND TEMPERATURE FOR THE ESA_moisture_winter X TEMPERATURE INTERACTION MODEL
season_narm_r <- season_narm_r[!is.na(season_narm_r$ESA_moisture_winter),]
...
ESA_moisture_winter_cent_amount <- attr(scale(season_narm_r$ESA_moisture_winter, center = TRUE, scale = FALSE), 'scaled:center')
season_narm_r$ESA_moisture_winter<-scale(season_narm_r$ESA_moisture_winter, center = TRUE, scale = FALSE)
...
season_narm_r_sites<-season_narm_r %>%
  group_by(SiteNum) %>%
  summarise(ESA_moisture_winter_site = mean(ESA_moisture_winter),
            ESA_moisture_winter_sd_site = sd(ESA_moisture_winter))

season_narm_r$ESA_moisture_winter_site<-season_narm_r_sites$ESA_moisture_winter_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]
season_narm_r$ESA_moisture_winter_sd_site<-season_narm_r_sites$ESA_moisture_winter_sd_site[match(season_narm_r$SiteNum, season_narm_r_sites$SiteNum)]

season_narm_r$ESA_moisture_winter_sd_site[season_narm_r$ESA_moisture_winter_sd_site==0 ] <- mean(season_narm_r$ESA_moisture_winter_sd_site[season_narm_r$ESA_moisture_winter_sd_site>0],na.rm = T)
season_narm_r$ESA_moisture_winter_sd_site[is.na(season_narm_r$ESA_moisture_winter_sd_site)] <- 0.01
...
season_narm_r_regions<-season_narm_r %>%
  group_by(RegionNum) %>%
  summarise(ESA_moisture_winter_region = mean(ESA_moisture_winter),
            ESA_moisture_winter_sd_region = sd(ESA_moisture_winter))

season_narm_r$ESA_moisture_winter_region<-season_narm_r_regions$ESA_moisture_winter_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]
season_narm_r$ESA_moisture_winter_sd_region<-season_narm_r_regions$ESA_moisture_winter_sd_region[match(season_narm_r$RegionNum, season_narm_r_regions$RegionNum)]

season_narm_r$ESA_moisture_winter_sd_region[season_narm_r$ESA_moisture_winter_sd_region==0] <- mean(season_narm_r$ESA_moisture_winter_sd_region[season_narm_r$ESA_moisture_winter_sd_region>0],na.rm = T)
season_narm_r$ESA_moisture_winter_sd_region[is.na(season_narm_r$ESA_moisture_winter_sd_region)] <- 0.01
...
min_soil<-min(season_narm_r$ESA_moisture_winter,na.rm=TRUE)
max_soil<-max(season_narm_r$ESA_moisture_winter,na.rm=TRUE)
min_ESA_moisture_winter<-min(season_narm_r$ESA_moisture_winter,na.rm=TRUE)
max_ESA_moisture_winter<-max(season_narm_r$ESA_moisture_winter,na.rm=TRUE)

xhats <- expand.grid(xhat1=seq(min_ESA_moisture_winter, max_ESA_moisture_winter,by=0.01), xhat3 = mean_burial) #AB: predicting soil ESA_moisture_winter at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
jags.dat<-list(
  ...
  #temp_plot=as.numeric(season_narm_r[!duplicated(season_narm_r$PlotNum),]$ESA_moisture_winter),
  #temp_site=as.numeric(season_narm_r[!duplicated(season_narm_r$SiteNum),]$ESA_moisture_winter),
  temp_mean_region=as.numeric(season_narm_r[!duplicated(season_narm_r$RegionNum),]$ESA_moisture_winter_region),
  temp_sd_region=as.numeric(season_narm_r[!duplicated(season_narm_r$RegionNum),]$ESA_moisture_winter_sd_region),
  temp_mean_site=as.numeric(season_narm_r[!duplicated(season_narm_r$SiteNum),]$ESA_moisture_winter_site),
  temp_sd_site=as.numeric(season_narm_r[!duplicated(season_narm_r$SiteNum),]$ESA_moisture_winter_sd_site),
  ...
  meanT=mean(as.numeric(season_narm_r$ESA_moisture_winter[!duplicated(season_narm_r$ESA_cell)])),
  xhat1=xhats$xhat1,
  xhat3=xhats$xhat3,
...
write("
      ...
      real<lower=-2,upper=2> gamma2[NTea];  // slope of ESA_moisture_winter - loss relationship
      real<lower=-2,upper=2> gamma3[NTea];  // temperature - ESA_moisture_winter interaction
      real<lower=-2,upper=2> gamma4[NTea];  // temperature - ESA_moisture_winter interaction
      ...
      //Relationship between mass loss at the region level and temperature and ESA_moisture_winter, per tea type
      ...
      ","scripts/users/hthomas/Tea/ESA_moisture_winter_loss_3.stan")



stanc('scripts/users/hthomas/Tea/ESA_moisture_winter_loss_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/ESA_moisture_winter_loss_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
#gamma1 = temperature, gamma2 = ESA_moisture_winter, gamma3 = temp X ESA_moisture_winter interaction (for each tea type)
...
predsout.space$ESA_moisture_winter <- rep(jags.dat$xhat1, each=2)
predsout.space$MostureBT <- predsout.space$ESA_moisture_winter + ESA_moisture_winter_cent_amount
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_winter_preds_winter.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_winter_fits_winter.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/ESA_moisture_winter_med_winter.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=ESA_moisture_winter+ESA_moisture_winter_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=ESA_moisture_winter+ESA_moisture_winter_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=ESA_moisture_winter+ESA_moisture_winter_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
  coord_cartesian(y = c(0,1))+
  scale_colour_manual(values = c("darkgreen","red3"), name = "Tea Type")+
  scale_fill_manual(values = c("darkgreen","red3"), name = "Tea Type")+
  scale_linetype_manual(values = c("dashed","solid"), name = "ESA_moisture_winter", labels = c("low","high"))+
...
lm <- lmer(Loss ~ ESA_moisture_winter * Tea_Type + (ESA_moisture_winter|ESA_cell/Site/Plot), data = season_narm_r)
out<-as.data.frame(effect(c("ESA_moisture_winter","Tea_Type"),lm))



(ESA_moisture_winter_only_winter<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = ESA_moisture_winter+ESA_moisture_winter_cent_amount, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = ESA_moisture_winter+ESA_moisture_winter_cent_amount, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=ESA_moisture_winter+ESA_moisture_winter_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=ESA_moisture_winter+ESA_moisture_winter_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=ESA_moisture_winter+ESA_moisture_winter_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(ESA_moisture_winter_only_winter, file = "scripts/users/hthomas/Tea/ESA_moisture_winter_only_winter.Rdata")

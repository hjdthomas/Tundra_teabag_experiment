...
season_narm<-summer %>%
  filter(is.finite(TBI_S),is.finite(CHELSA_summer_temp))
...
write("
      ...
      real<lower=-2,upper=2> gamma2;  // slope of CHELSA_summer_temp - TBI_S relationship
      ...
      //Relationship between mass TBI_S at the region level and temperature and CHELSA_summer_temp, per tea type
      ...
      ","scripts/users/hthomas/Tea/CHELSA_summer_temp_TBI_S_3.stan")



stanc('scripts/users/hthomas/Tea/CHELSA_summer_temp_TBI_S_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/CHELSA_summer_temp_TBI_S_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_temp_preds_summer_TBI_S.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_summer_temp_fits_summer_TBI_S.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/CHELSA_summer_temp_med_summer_TBI_S.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=CHELSA_summer_temp+CHELSA_summer_temp_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
dev.off()
library(effects)
lm <- lmer(TBI_S ~ CHELSA_summer_temp  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("CHELSA_summer_temp"),lm))

(CHELSA_summer_temp_only_summer_TBI_S<-ggplot()+
...
save(CHELSA_summer_temp_only_summer_TBI_S, file = "scripts/users/hthomas/Tea/CHELSA_summer_temp_only_summer_TBI_S.Rdata")

...
season_narm<-summer %>%
  filter(is.finite(TBI_S),is.finite(moisture_mean))
...
write("
      ...
      real<lower=-2,upper=2> gamma2;  // slope of moisture - TBI_S relationship
      real<lower=-2,upper=2> gamma3;  // temperature - moisture interaction
      real<lower=-2,upper=2> gamma4;  // temperature - moisture interaction
      ...
      //Relationship between mass TBI_S at the region level and temperature and moisture, per tea type
    
      
      for (i in 1:NSite){
      if(multsites_lsite[i] == 1 && site_envlevel[i] >0)
      as[i] ~ normal(gamma0 + gamma1*temp_pred_site[i], sigma_overall); 
      ...
      else aMeanRegion[i] ~ normal(gamma0 + gamma1*temp_pred_region[i], sigma_overall); 
      }
      ...
      ","scripts/users/hthomas/Tea/moisture_TBI_S_3.stan")



stanc('scripts/users/hthomas/Tea/moisture_TBI_S_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/moisture_TBI_S_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
predsout.space$moisture <-jags.dat$xhat1
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/moisture_preds_summer_TBI_S.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/moisture_fits_summer_TBI_S.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/moisture_med_summer_TBI_S.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=moisture+moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=moisture_mean+moisture_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=moisture+moisture_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
dev.off()
library(effects)
lm <- lmer(TBI_S ~ moisture_mean  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("moisture_mean"),lm))

(moisture_only_summer_TBI_S<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = moisture_mean+moisture_cent_amount, ymin = lower, ymax = upper),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = moisture_mean+moisture_cent_amount, y = fit),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=moisture+moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=moisture_mean+moisture_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=moisture+moisture_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(moisture_only_summer_TBI_S, file = "scripts/users/hthomas/Tea/moisture_only_summer_TBI_S.Rdata")

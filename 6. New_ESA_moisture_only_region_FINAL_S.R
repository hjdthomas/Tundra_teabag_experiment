...
season_narm<-summer %>%
  filter(is.finite(TBI_S),is.finite(ESA_moisture))
...
# AB: REMOVE MISSING VALUES OF SOIL ESA_moisture AND TEMPERATURE FOR THE ESA_moisture X TEMPERATURE INTERACTION MODEL
...
min_soil<-min(season_narm_r$ESA_moisture,na.rm=TRUE)
max_soil<-max(season_narm_r$ESA_moisture,na.rm=TRUE)
min_ESA_moisture<-min(season_narm_r$ESA_moisture,na.rm=TRUE)
max_ESA_moisture<-max(season_narm_r$ESA_moisture,na.rm=TRUE)

xhats <- expand.grid(xhat1=seq(min_ESA_moisture, max_ESA_moisture,by=0.01), xhat3 = mean_burial) #AB: predicting soil ESA_moisture at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
write("
      ...
      real<lower=-2,upper=2> gamma2;  // slope of ESA_moisture - TBI_S relationship
      real<lower=-2,upper=2> gamma3;  // temperature - ESA_moisture interaction
      real<lower=-2,upper=2> gamma4;  // temperature - ESA_moisture interaction
      ...
      //Relationship between mass TBI_S at the region level and temperature and ESA_moisture, per tea type
      ...
      ","scripts/users/hthomas/Tea/ESA_moisture_TBI_S_3.stan")



stanc('scripts/users/hthomas/Tea/ESA_moisture_TBI_S_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/ESA_moisture_TBI_S_3.stan', data = jags.dat, init=inits, iter = 15000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
#gamma1 = temperature, gamma2 = ESA_moisture, gamma3 = temp X ESA_moisture interaction (for each tea type)
...
predsout.space$ESA_moisture <-jags.dat$xhat1
predsout.space$MostureBT <- predsout.space$ESA_moisture + ESA_moisture_cent_amount
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_preds_summer_TBI_S.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_fits_summer_TBI_S.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/ESA_moisture_med_summer_TBI_S.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
dev.off()
library(effects)
lm <- lmer(TBI_S ~ ESA_moisture  + (1|ESA_cell/Site), data = season_narm_r)
out<-as.data.frame(effect(c("ESA_moisture"),lm))

(ESA_moisture_only_summer_TBI_S<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = ESA_moisture+ESA_moisture_cent_amount, ymin = lower, ymax = upper),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = ESA_moisture+ESA_moisture_cent_amount, y = fit),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`)),fill = "darkgreen",alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=TBI_S),colour="darkgreen",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=mean), colour = "darkgreen", alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(ESA_moisture_only_summer_TBI_S, file = "scripts/users/hthomas/Tea/ESA_moisture_only_summer_TBI_S.Rdata")

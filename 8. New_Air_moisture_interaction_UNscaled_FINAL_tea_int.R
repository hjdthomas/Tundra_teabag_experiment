...
airtemp_scale_amount <- attr(scale(season_narm_r$airtemp_mean, center = TRUE, scale = FALSE), 'scaled:scale')
...
moist_x_scale_amount <- attr(scale(c(LQ_moist,UQ_moist,mean_moist,med_moist), center = TRUE, scale = FALSE), 'scaled:scale')
...
predsout.space$TempBT <- predsout.space$airtemp  + airtemp_cent_amount 
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_moisture_preds_tea_int_unscaled.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_moisture_fits_tea_int_unscaled.Rdata")

#Anne graph
pdf("scripts/users/hthomas/Output_Images/Tea/airtemp_moisture_unscaled.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp +airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(Moisture)),alpha=0.2)+
  geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=airtemp_mean+airtemp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
  geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=airtemp_mean+airtemp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp+airtemp_cent_amount,y=mean, colour=factor(Tea_Type):factor(Moisture)), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
(airtemp_moist_tea_int_unscaled<-ggplot()+
    geom_ribbon(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp +airtemp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(Moisture)),alpha=0.2)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=airtemp_mean+airtemp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=airtemp_mean+airtemp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space[predsout.space$MoistureBT==LQ_moist | predsout.space$MoistureBT==UQ_moist,],aes(x=airtemp+airtemp_cent_amount,y=mean, colour=factor(Tea_Type):factor(Moisture)), alpha=0.8, lwd = 1.5)+
...
save(airtemp_moist_tea_int_unscaled, file = "scripts/users/hthomas/Tea/airtemp_moist_tea_int_unscaled.Rdata")

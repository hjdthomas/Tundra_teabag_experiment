...
season_narm_r_plots<-season_narm_r %>%
  group_by(PlotNum) %>%
  summarise(moisture_mean_plot = mean(moisture_mean),
            moisture_sd_plot = sd(moisture_mean))

season_narm_r$moisture_mean_plot<-season_narm_r_plots$moisture_mean_plot[match(season_narm_r$PlotNum, season_narm_r_plots$PlotNum)]
season_narm_r$moisture_sd_plot<-season_narm_r_plots$moisture_sd_plot[match(season_narm_r$PlotNum, season_narm_r_plots$PlotNum)]

season_narm_r$moisture_sd_plot[season_narm_r$moisture_sd_plot==0] <- mean(season_narm_r$moisture_sd_plot[season_narm_r$moisture_sd_plot>0],na.rm = T)
season_narm_r$moisture_sd_plot[is.na(season_narm_r$moisture_sd_plot)] <- 0.01
...
xhats <- expand.grid(xhat1=seq(min_moisture, max_moisture,by=0.01), xhat3 = mean_burial) #AB: predicting air moisture at 25% and 75% (assuming you will graph temperature as continuous) but of course you can change this to whatever you want
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/moisture_preds_within.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/moisture_fits_within.Rdata")


pdf("scripts/users/hthomas/Output_Images/Tea/moisture_within.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=moisture,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=moisture_mean,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=moisture,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
(moisture_within<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = moisture_mean, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = moisture_mean, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=moisture,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=moisture_mean,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=moisture,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(moisture_within, file = "scripts/users/hthomas/Tea/moisture_within.Rdata")

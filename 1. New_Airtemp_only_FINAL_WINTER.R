...
season_narm<-winter %>%
  filter(is.finite(winter[,var.num]),is.finite(airtemp_mean))
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_preds_winter.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/airtemp_fits_winter.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/airtemp_only_winter.pdf", width = 3, height = 3)
ggplot()+
...
save(airtemp_only_winter, file = "scripts/users/hthomas/Tea/airtemp_only_winter.Rdata")

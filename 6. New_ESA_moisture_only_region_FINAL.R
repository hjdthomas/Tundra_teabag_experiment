...
write("
      ...
      real<lower=-2,upper=2> gamma2[NTea];  // slope of ESA_moisture - loss relationship
      real<lower=-2,upper=2> gamma3[NTea];  // temperature - ESA_moisture interaction
      real<lower=-2,upper=2> gamma4[NTea];  // temperature - ESA_moisture interaction
      ...
      //Relationship between mass loss at the region level and temperature and ESA_moisture, per tea type
      ...
      ","scripts/users/hthomas/Tea/ESA_moisture_loss_3.stan")



stanc('scripts/users/hthomas/Tea/ESA_moisture_loss_3.stan') #check model
...
fit_space <- stan(file = 'scripts/users/hthomas/Tea/ESA_moisture_loss_3.stan', data = jags.dat, init=inits, iter = 25000, chains = 2, thin = 1, verbose = TRUE, control=list(adapt_delta=0.99,max_treedepth = 15), algorithm = "NUTS") 
...
predsout.space$ESA_moisture <- rep(jags.dat$xhat1, each=2)
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_preds_summer.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/ESA_moisture_fits_summer.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/ESA_moisture_med_summer.pdf", width = 3, height = 3)
ggplot()+
  geom_ribbon(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
  geom_point(data=season_narm_r,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
lm <- lmer(Loss ~ ESA_moisture * Tea_Type + (ESA_moisture|ESA_cell/Site/Plot), data = season_narm_r)
out<-as.data.frame(effect(c("ESA_moisture","Tea_Type"),lm))



(ESA_moisture_only_summer<-ggplot()+
    geom_ribbon(data = out, mapping = aes(x = ESA_moisture+ESA_moisture_cent_amount, ymin = lower, ymax = upper, group = Tea_Type),fill="grey", alpha=0.5) +
    geom_line(data = out, mapping = aes(x = ESA_moisture+ESA_moisture_cent_amount, y = fit, group = Tea_Type),colour = "grey") +
    geom_ribbon(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type)),alpha=0.2)+
    geom_point(data=season_narm_r,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=Loss,colour=factor(Tea_Type)),pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space,aes(x=ESA_moisture+ESA_moisture_cent_amount,y=mean, colour = Tea_Type), alpha=0.8, lwd = 1.5)+
    theme_classic()+
...
save(ESA_moisture_only_summer, file = "scripts/users/hthomas/Tea/ESA_moisture_only_summer.Rdata")

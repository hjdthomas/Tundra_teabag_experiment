...
CHELSA_summer_temp_cent_amount <- attr(scale(season_narm_r$CHELSA_summer_temp, center = TRUE, scale = TRUE), 'scaled:center')
CHELSA_summer_temp_scale_amount <- attr(scale(season_narm_r$CHELSA_summer_temp, center = TRUE, scale = TRUE), 'scaled:scale')
season_narm_r$CHELSA_summer_temp<-scale(season_narm_r$CHELSA_summer_temp, center = TRUE, scale = TRUE)
ESA_moisture_cent_amount <- attr(scale(season_narm_r$ESA_moisture, center = TRUE, scale = TRUE), 'scaled:center')
ESA_moisture_scale_amount <- attr(scale(season_narm_r$ESA_moisture, center = TRUE, scale = TRUE), 'scaled:scale')
season_narm_r$ESA_moisture<-scale(season_narm_r$ESA_moisture, center = TRUE, scale = TRUE)
...
ESA_moisture_x_cent_amount <- attr(scale(c(LQ_moist,UQ_moist,mean_moist,med_moist), center = TRUE, scale = TRUE), 'scaled:center')
ESA_moisture_x_scale_amount <- attr(scale(c(LQ_moist,UQ_moist,mean_moist,med_moist), center = TRUE, scale = TRUE), 'scaled:scale')
xhats <- expand.grid(xhat1=seq(min_CHELSA_summer_temp, max_CHELSA_summer_temp,by=0.01), xhat2=scale(c(LQ_moist,UQ_moist,mean_moist,med_moist), center = TRUE, scale = TRUE), xhat3 = mean_burial) #AB: predicting air CHELSA_summer_temp at 25% and 75% (assuming you will graph CHELSA_summer_temperature as continuous) but of course you can change this to whatever you want
...
predsout.space$CHELSA_summer_tempBT <- predsout.space$CHELSA_summer_temp*CHELSA_summer_temp_scale_amount + CHELSA_summer_temp_cent_amount 
predsout.space$ESA_moisture <- rep(jags.dat$xhat2, each=2)
predsout.space$ESA_moistureBT <- predsout.space$ESA_moisture*ESA_moisture_x_scale_amount + ESA_moisture_x_cent_amount
...
save(predsout.space, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_ESA_preds_scaled.Rdata")
save(cout, file = "scripts/users/hthomas/Tea/Stan_outputs/CHELSA_ESA_fits_scaled.Rdata")

pdf("scripts/users/hthomas/Output_Images/Tea/CHELSA_ESA_interaction_scaled.pdf", width = 4, height = 4)
ggplot()+
  geom_ribbon(data=predsout.space[predsout.space$ESA_moistureBT==LQ_moist | predsout.space$ESA_moistureBT==UQ_moist,],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(ESA_moisture)),alpha=0.2)+
  geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
  geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
  geom_line(data=predsout.space[predsout.space$ESA_moistureBT==LQ_moist | predsout.space$ESA_moistureBT==UQ_moist,],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,y=mean, colour=factor(Tea_Type):factor(ESA_moisture)), alpha=0.8, lwd = 1.5)+
  theme_classic()+
...
(CH_ESA_interaction_scaled<-ggplot()+
    geom_ribbon(data=predsout.space[predsout.space$ESA_moistureBT==LQ_moist | predsout.space$ESA_moistureBT==UQ_moist,],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,ymin=(`2.5%`),ymax=(`97.5%`),fill=factor(Tea_Type):factor(ESA_moisture)),alpha=0.2)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Green",],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,y=Loss),colour = "#006400",pch =16 ,alpha=0.6)+
    geom_point(data=season_narm_r[season_narm_r$Tea_Type=="Rooibos",],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,y=Loss), colour = "#8B2323",pch =16 ,alpha=0.6)+
    geom_line(data=predsout.space[predsout.space$ESA_moistureBT==LQ_moist | predsout.space$ESA_moistureBT==UQ_moist,],aes(x=CHELSA_summer_temp*CHELSA_summer_temp_scale_amount +CHELSA_summer_temp_cent_amount,y=mean, colour=factor(Tea_Type):factor(ESA_moisture)), alpha=0.8, lwd = 1.5)+
...
save(CH_ESA_interaction_scaled, file = "scripts/users/hthomas/Tea/CH_ESA_interaction_scaled.Rdata")
